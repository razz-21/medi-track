<script lang="ts" module>
	...
	type Props = {
		vaccinedBrgyStats: DashboardStatsGet['vaccined_brgy_stats'];
	};
</script>

<script lang="ts">
	let { vaccinedBrgyStats }: Props = $props();
	...
	let data = $derived(() => {
		...
		brgys.forEach((brgy) => {
			const brgyData = vaccinedBrgyStats.find((brgyData) => brgyData.brgy === brgy);
...
<div class="w-full border border-gray-200 rounded-lg p-4">
	<h3 class="text-sm font-bold">Vaccinated per brgy</h3>
